...
for i in range(15):
    bin_size, number_of_instances, dict = firstFitDecreasing.readInInstances(path + "medium\instance" + str(i) + ".txt")
    t = timeit.Timer(lambda: f1(dict, bin_size))
    print("Time to solve instance" + str(i) + ": " + str(t.timeit(number=1)))

print("Solving large instances...")
for i in range(15):
    bin_size, number_of_instances, dict = firstFitDecreasing.readInInstances(path + "large\instance" + str(i) + ".txt")
    t = timeit.Timer(lambda: f1(dict, bin_size))
    print("Time to solve instance" + str(i) + ": " + str(t.timeit(number=1)))
